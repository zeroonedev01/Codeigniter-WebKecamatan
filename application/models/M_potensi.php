...
class M_potensi extends CI_Model {
	function get_all_potensi() {
		$hsl = $this->db->query("SELECT tb_potensi.*,DATE_FORMAT(tanggal,'%d/%m/%Y') AS tanggal FROM tb_potensi ORDER BY id DESC");
		return $hsl;
	}
	function tampil_potensi() {
		$hsl = $this->db->query("SELECT tb_potensi.id, tb_potensi.judul, tb_potensi.tanggal, tb_potensi.isi, tb_potensi.slug,tb_potensi.gambar,tb_user.nama AS author, DATE_FORMAT(tanggal,'%d/%m/%Y') AS tanggal FROM tb_potensi INNER join tb_user on tb_potensi.userid = tb_user.id ORDER BY id DESC");
		return $hsl;
	}
	...
	function potensi_perpage($offset, $limit) {
		$hsl = $this->db->query("SELECT tb_potensi.*,DATE_FORMAT(tanggal,'%d %M %Y %h:%i') AS tanggal FROM tb_potensi ORDER BY id DESC limit $offset,$limit");
		return $hsl;
	}
	...
	function get_potensid_by_kode($kode) {
		$hsl = $this->db->query("SELECT tb_potensi.id, tb_potensi.judul, tb_potensi.tanggal, tb_potensi.isi, tb_potensi.slug,tb_potensi.gambar,tb_user.nama AS author, DATE_FORMAT(tanggal,'%d %M %Y %h:%i') AS tanggal FROM tb_potensi INNER join tb_user on tb_potensi.userid = tb_user.id where tb_potensi.id='$kode'");
		return $hsl;
	}

	function cari_potensi($keyword) {
		$hsl = $this->db->query("SELECT tb_potensi.*,DATE_FORMAT(tanggal,'%d/%m/%Y') AS tanggal FROM tb_potensi WHERE judul LIKE '%$keyword%' LIMIT 5");
		return $hsl;
	}
