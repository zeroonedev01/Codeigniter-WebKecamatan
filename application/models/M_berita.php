...
class M_berita extends CI_Model {
	function get_all_berita() {
		$hsl = $this->db->query("SELECT tb_berita.*,DATE_FORMAT(tanggal,'%d/%m/%Y') AS tanggal FROM tb_berita ORDER BY id DESC");
		return $hsl;
	}
	...
	function simpan_berita($judul, $isi, $kategoriid, $userid, $gambar, $slug) {
		$hsl = $this->db->query("insert into tb_berita(judul,isi,kategoriid,userid,gambar,slug) values ('$judul','$isi','$kategoriid','$userid','$gambar','$slug')");
		return $hsl;
	}
	...
	function update_berita($id, $judul, $isi, $kategoriid, $userid, $gambar, $slug) {
		$hsl = $this->db->query("update tb_berita set judul='$judul',isi='$isi',kategoriid='$kategoriid',userid='$userid',gambar='$gambar',slug='$slug' where id='$id'");
		return $hsl;
	}
	function update_berita_tanpa_img($id, $judul, $isi, $kategoriid, $userid, $slug) {
		$hsl = $this->db->query("update tb_berita set judul='$judul',isi='$isi',kategoriid='$kategoriid',userid='$userid',slug='$slug' where id='$id'");
		return $hsl;
	}
	...
	function cari_berita($keyword) {
		$hsl = $this->db->query("SELECT tb_berita.id, tb_berita.judul, tb_berita.tanggal, tb_berita.isi, tb_berita.tayang,tb_berita.slug,tb_berita.gambar,tb_kategori.nama as kategori ,tb_user.nama AS author, DATE_FORMAT(tanggal,'%d/%m/%Y') AS tanggal FROM tb_berita INNER JOIN tb_kategori on tb_berita.kategoriid = tb_kategori.id INNER join tb_user on tb_berita.userid = tb_user.id WHERE judul LIKE '%$keyword%' LIMIT 5");
		return $hsl;
	}
